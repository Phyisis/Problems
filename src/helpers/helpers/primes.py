...
def primes(n):
    ...
    for p in bitmap.iter_clear(3, gmpy2.isqrt(n) + 1):
        bitmap[p*p:n:p+p] = -1
    return list(bitmap.iter_clear(2,n+1))
...
class dynamicPrimes():
    ...
    def expect(self,n):
        if n > self.n:
            self.primes += ballieExtend(self.n,n+1)
            self.n = n
        return self.primes
...
if __name__ == "__main__":
    #print(primeslist.expect(11))
    #print(primeslist.primes,analytics.lap(),analytics.maxMem())
